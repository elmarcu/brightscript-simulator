...
- **Compiles** BrighterScript → BrightScript using `bsc` (from the `brighterscript` package).
...
```
.
├── Dockerfile                   # Node 18 + brighterscript + brs
├── docker-compose.yml          # Service config with hot-reload mounts
├── server.js                   # Express server + watcher + compile/execute orchestration
├── package.json                # Dependencies: express, brighterscript, brs, chokidar
├── public/                     # Static web UI (build log, file viewer, Execute button)
├── projects/
│   ├── hello-world/            # Simple console output example
│   │   ├── bsconfig.json       # BrighterScript compiler config
│   │   ├── manifest            # Roku manifest (metadata)
│   │   ├── source/             # BrighterScript source (.bs files)
│   │   │   ├── main.bs
│   │   │   └── greet.bs
│   │   └── dist/               # Generated by bsc (staging dir)
│   └── hello-screen/           # UI scene example with ASCII rendering
│       ├── bsconfig.json       # BrighterScript compiler config
│       ├── manifest            # Roku manifest (metadata)
│       ├── source/             # BrighterScript source (.bs files)
│       │   └── main.bs
...
function RenderScene(scene as object) as void
    print "┌" + string(50, "─") + "┐"
    print "│ " + CenterText(scene.message, 48) + " │"
    print "├" + string(50, "─") + "┤"
...
┌──────────────────────────────────────────────────────┐
│ Welcome to BrightScript UI Demo! │
├──────────────────────────────────────────────────────┤
│ Hello Screen
│ ╔════════════════════════╗
│ ║       Start        ║
...
2. **Compile**: On file change, runs `npx bsc --project bsconfig.json` (with `cwd` set to the project folder) to transpile BrighterScript → BrightScript into `dist/source/`.
